...
export default function TatianaAssistant() {
  ...
  return <main className={styles.shell}><section className={styles.card}><header className={styles.header}><div className={styles.profile}><Image className={styles.profileImage} src="/assets/o1sf/tatianasf-assistant-share-card-v2.jpg" alt={t.profileAlt} width={1280} height={668} priority sizes="(max-width: 496px) calc(100vw - 68px), 420px" /><div className={styles.profileCopy}><h1 className={styles.profileName}>TatianaSF</h1><p className={styles.profileRole}>{t.profileRole}</p><p className={styles.profileDescriptor}>{t.profileDescriptor}</p><p className={styles.profileSummary}>{t.profileSummary}</p></div></div></header><div className={styles.conversation}>{history.map((item, i) => <div className={styles.history} key={`${item.answer}-${i}`}><div className={styles.user}>{item.answer}</div><button className={`${styles.action} ${styles.change}`} onClick={() => resetFrom(i)}>{t.change}</button></div>)}{text && <div className={step.id === "final" ? `${styles.bubble} ${styles.handoff}` : styles.bubble}>{text}</div>}{step.id === "final" && <div className={styles.handoffActions}><p className={styles.guidance} aria-live="polite">{copyStatus === "copied" ? t.copiedGuidance : t.copyGuidance}</p><button className={styles.copyButton} onClick={copyMessage} aria-live="polite">{copyStatus === "copied" ? t.copied : copyStatus === "failed" ? t.copyFailed : t.copy}</button><a className={styles.linkedinAction} href="https://www.linkedin.com/in/tatianasf" target="_blank" rel="noopener noreferrer">{t.linkedin}</a></div>}{choices.length > 0 && <div className={styles.choices}>{choices.map((key) => key === "findEvents" ? <a className={styles.choice} href="https://www.google.com/search?q=TatianaSF+lu.ma" key={key} rel="noopener noreferrer" style={{ alignItems: "center", display: "flex", minHeight: 44 }} target="_blank">{t.findEvents}</a> : <button className={styles.choice} key={key} onClick={() => choose(key)}>{step.id === "role" ? key : label(t.choices[key] || t[key] || key)}</button>)}</div>}</div><footer className={styles.footer}>Choose a reply to continue.</footer></section></main>;
}
